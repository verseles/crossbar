...
7. **Strict Operational Rules:** Adhere to the specific operational rules imported from `AGENTS.md`.
...
6. **Verify Coverage & Quality (Mandatory):**
   - **Static Analysis:** Run `make analyze`. Fix all errors.
   - **Coverage:** Run `make coverage`.
   - **Target:** >60% coverage (excluding generated code).
   - **Platform Builds:** Run `make linux` and `make android` to ensure no platform-specific breakages (Github Actions CI is slower).
...
8. **Commit Code Changes:**
   ...
   - **Constraint:** Use Conventional Commits (`feat`, `fix`, `docs`, `test`, `ci`). **NO co-authors**.
   - Perform the commit.
...
3.  **Execute Automated Tests with Proactive Debugging:**
    ...
    -   **Example Announcement:** "I will now run the automated test suite to verify the phase. **Command:** `make precommit` (wraps analyze, coverage, builds)"
...
- [ ] All tests pass (`make precommit` success)
...
- [ ] **Dependencies Checked:** `pubspec.yaml` verified (never assume libraries).
...
make precommit
# OR run manually:
make analyze && make coverage && make linux && make android
```
...
3. **Testing**
   ...
   - **Platform Checks:** Linux and Android builds pass.
...
6. **Platform Builds:** Linux and Android builds pass.
...
- [ ] Backup created
- [ ] **Local Verification:** Confirmed via `act` or local build commands.
