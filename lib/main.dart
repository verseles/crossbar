...
void main(List<String> args) async {
  ...
  try {
    ...
    if (!ipcStarted) {
      logger.info(
        'IPC server failed to start (port busy). Another instance is likely running.',
      );
      ...
      if (!startMinimized) {
        ...
        try {
          ...
          final request = await client.getUrl(
            Uri.parse('http://localhost:${IpcServer.defaultPort}/window/show'),
          );
          ...
          if (response.statusCode == HttpStatus.ok) {
            ...
            logger.info(
              'Signal sent but received status ${response.statusCode}',
            );
          }
        } catch (e) {
    ...
    // Discover plugins
    await SamplePluginsService().syncInstalledSamples(
      force: Platform.isAndroid || Platform.isIOS,
    );
